...
# 				self.c = True
			...
			#rot.roll() or self.pose.rotation().roll()????
			self.x = np.array([[self.pose.x()], [self.pose.y()], [self.pose.z()], [rot.roll()], [rot.pitch()], [rot.yaw()], [vel[0]], [vel[1]], [vel[2]], [0], [0], [0]])

			self.x = self.pred(self.x,dt)

			trans = self.tolist(self.x[0:3])
